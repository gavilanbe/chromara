...
function* returnHome(u, n = 10) { u.pose = 'idle'; yield* whop(u, u.hx, u.hy, n, 8); u.wz = 0; }
...
function* atkBrocha(u, t) { // la brocha entra enorme por el borde, en primer plano, y cruza hasta el enemigo dejando un trazo de cerdas que lo tapa; se seca y se descascarilla
  const col = C(u.color); camFocus(t.wx, t.wy, { dist: 78, turn: -.3, h: 44 }); u.pose = 'attack'; const [ax, ay] = infront(u, t, t.def.w * .7 + 18); yield* whop(u, ax, ay, 8, 10);
  ...
  for (let i = 1; i <= 9; i++) { st.k = (i / 9) ** 1.4; yield; }
...
function* atkLapiz(u, t) { // el lápiz garabatea alrededor del enemigo a toda velocidad y lo tacha con dos rayas; saltan virutas
  ...
  const [ax, ay] = infront(u, t, t.def.w * .7 + 16); for (let i = 1; i <= 6; i++) { const k = i / 6; u.wx = lerp(u.hx, ax, k * (2 - k)); u.wy = lerp(u.hy, ay, k * (2 - k)); if (i % 2) gh.add(); yield; }
  ...
  yield* toolStroke({ kind: 'lapiz', color: col, pts, w: 2, col: GRAPHITE, under: GRAPHITE2, frames: 10, fac: -1, life: 60, jit: 1, scale: 1.1, onTip: (k, wp) => { if (Math.random() < .5) B.particles.push({ wx: wp[0], wy: wp[1], wz: wp[2], vx: R(-.4, .4), vy: 0, vz: R(.2, .8), g: .08, col: '#e8cf9a', t: 0, life: 14 }); } });
...
function* atkPincel(u, t) { // el pincel se moja en Añil (se agita), pinta una voluta en el aire que sale disparada como latigazo de tinta y salpica gotas que ruedan por el suelo
  ...
  Audio.sfx('brush_hiss'); for (let i = 0; i < 14; i++) { st.dip = i < 7 ? i * 3 : (14 - i) * 3; u.wz = i > 4 && i < 11 ? R(-1, 1) : 0; if (i > 5 && i % 2) drips(u.wx, u.wy, u.def.h * .6, col, 2); yield; }
...
function* techBrochazo(u, t, tech, col) { // la brocha cae desde arriba como un mazo y estampa una Z roja en tres golpes; el suelo bajo el enemigo queda pintado
  camFocus(t.wx, t.wy, { dist: 84, turn: -.3, h: 56, pitch: .7 }); u.pose = 'attack'; yield* whop(u, ...infront(u, t, t.def.w * .8 + 24), 8, 12);
...
function* techSalpicon(u, targets, tech, col) { // Añil salta, el pincel gira como aspa y llueven gotas azules en arco sobre todos, con sombras que anuncian dónde cae cada una
  ...
  for (let i = 0; i < 16; i++) { u.wz = Math.sin(i / 16 * Math.PI) * 44; st.a += .9; if (i > 4 && i % 2 === 0) for (const tt of targets) B.particles.push({ wx: u.wx, wy: u.wy, wz: u.wz + u.def.h, tx: tt.wx + R(-14, 14), ty: tt.wy + R(-8, 8), tz: 0, col: C(col), t: 0, life: 14, dur: 14, arc: 30, stream: true }); yield; }
...
function* fuseAt(users, wx, wy, wz, col) { const r = yield* chargeAndFuse(users, wx, wy, wz, col, 22); return r; }
function* techLlamarada(users, targets, tech, col) { // lápiz y brocha se cruzan como una cerilla, el lápiz raspa, prende, y un fuego naranja de pintura recorre el suelo hasta los enemigos
  ...
  Audio.sfx('scratch_long'); for (let i = 1; i <= 8; i++) { st.k = i / 8; B.particles.push({ wx: mid[0] + R(-3, 3), wy: mid[1], wz: 40 + R(0, 8), vx: R(-.5, .5), vy: 0, vz: R(.5, 1.5), g: .06, col: i & 1 ? C('amarillo') : C('naranja'), t: 0, life: 12 }); yield; }
...
function* techBrote(users, targets, tech, col) { // el pincel pinta un tallo desde el suelo que trepa por el enemigo y florece; el polen cura al grupo
  ...
  for (let i = 1; i <= 6; i++) { orb.wz = lerp(60, 2, i / 6); orb.r = 10 - i; yield; } of.dur = 0; Audio.sfx('grow'); Audio.sfx('leaves', { when: .3 });
  ...
  for (const t of targets) { // tallo pintado con el pincel
    ...
    yield* toolStroke({ kind: 'pincel', color: C(col), pts, w: 3, col: '#2f9a48', frames: 8, fac: -1, life: 130, scale: 1, onTip: (k, p) => { if (Math.random() < .5) mark({ kind: 'blob', p: [p[0] + R(-3, 3), p[1], p[2]], w: 3, col: C(col), grow: 3, life: 120, seed: (k * 99) | 0 }); } });
    ...
    sparkle(top[0], top[1], top[2] + 4, C('amarillo'));
  }
...
function* techEclipse(users, t, tech, col) { // la pantalla se oscurece, un disco rojo y otro azul se superponen, y el violeta cae como un tampón de tinta que deja la silueta del enemigo estampada
  ...
  for (let i = 1; i <= 22; i++) { const k = i / 22; discs.forEach((d, j) => { d.wx = lerp(d.u.wx, goal[0] + (j ? 1 : -1) * 18 * (1 - k), k); d.wy = lerp(d.u.wy, goal[1], k); d.wz = lerp(d.u.def.h + 10, goal[2], k); d.r = 6 + k * 10; }); yield; }
...
function* techArcoiris(users, targets, tech, col) { // los tres orbitan, la pantalla se vuelve papel blanco, un prisma dibuja una cinta de arcoíris que barre el campo y colorea a los enemigos hasta borrarlos
  ...
  for (let i = 0; i < 40; i++) { const k = i / 40; users.forEach((u, j) => { const a = k * k * 14 + j * 2.09, r = lerp(30, 12, k); u.wx = lerp(home[j][0], mid[0] + Math.cos(a) * r, Math.min(1, k * 3)); u.wy = lerp(home[j][1], mid[1] + Math.sin(a) * r * .6, Math.min(1, k * 3)); u.wz = 20 + Math.sin(a) * 6; if (i % 2 === 0) B.particles.push({ wx: u.wx, wy: u.wy, wz: u.wz + 8, vx: 0, vy: 0, vz: .6, g: -.01, col: C(u.color), t: 0, life: 14 }); }); yield; }
...
function* actItem(u, itemId, target) {
  ...
  camFocus(target.wx, target.wy, { dist: 80, turn: target.kind === 'party' ? .4 : -.2, h: 46 }); u.pose = 'attack'; yield* wait(6);
  if (it.kind === 'heal') { // una nube dibujada a lápiz llueve sobre el aliado
    ...
    for (let i = 0; i < 24; i++) { if (i % 2 === 0) B.particles.push({ wx: target.wx + R(-12, 12), wy: target.wy + R(-4, 4), wz: target.def.h + 30, vx: 0, vy: 0, vz: -R(1, 2), g: .1, col: C('azul'), t: 0, life: 20, size: 2 }); if (i === 12) { Audio.sfx('splash_clean'); heal(target, it.amount); goop(target, C('azul'), 40); } yield; }
    cl.dur = 0;
  ...
  u.pose = 'idle'; yield* wait(10); camReset();
}
// =====================================================================
...
function* enemyAttack(u, t) {
  ...
  if (shape === 'round') { // salta alto y se estampa contra el objetivo (planchazo)
    u.pose = 'attack'; Audio.sfx('charge', { semi: -5 }); const sh = shadowFx(t.wx, t.wy, 4, 999);
    for (let i = 1; i <= 14; i++) { const k = i / 14; u.wx = lerp(u.hx, t.wx, k); u.wy = lerp(u.hy, t.wy - 2, k); u.wz = Math.sin(k * Math.PI) * 60; sh.draw = () => { const c = project(t.wx, t.wy, 0); if (c) shadow(c[0], c[1], Math.round((4 + k * 16) * c[2])); }; yield; }
    sh.dur = 0; u.wz = 0; B.shake = 5; B.hitstop = 3; Audio.sfx('splash'); hitBasic(u, t); goop(t, C('negro'), 60); mark({ kind: 'blob', p: [t.wx, t.wy, 0], w: 12, col: C('negro'), seed: 7, life: 90, under: true });
    yield* wait(8); yield* whop(u, u.hx, u.hy, 10, 20);
  } else if (shape === 'splash') { // escupe tres pegotes en arco
    u.pose = 'attack'; for (let k = 0; k < 3; k++) { Audio.sfx('ink_jet', { pan: -.3 }); const p = { wx: u.wx, wy: u.wy, wz: u.def.h * .5, tx: t.wx + R(-4, 4), ty: t.wy + R(-3, 3), tz: t.def.h * .5, col: C('negro'), t: 0, life: 12, dur: 12, arc: 26, stream: true, size: 3 }; B.particles.push(p); u.wz = 4; yield* wait(4); u.wz = 0; yield* wait(6); burst(t.wx, t.wy, t.def.h * .5, C('negro'), 5, 1.2, 14, .1); if (k === 2) { hitBasic(u, t); goop(t, C('negro'), 60); } else { t.pose = 'hurt'; t.poseT = 6; } }
    yield* wait(8);
  } else if (shape === 'tall') { // embiste atravesando al objetivo y deja un tachón en el suelo
    u.pose = 'attack'; Audio.sfx('dash'); const gh = ghostsOf(u); const to = fwdOf(t, -22);
    ...
    for (let i = 1; i <= 8; i++) { const k = i / 8; u.wx = lerp(u.hx, to[0], k); u.wy = lerp(u.hy, to[1], k); gh.add(); if (i === 5) { hitBasic(u, t); goop(t, C('negro'), 60); } yield; }
    gh.end(); yield* wait(6); yield* whop(u, u.hx, u.hy, 10, 12);
  } else { // charco: se aplasta y manda una ola de tinta por el suelo
    u.pose = 'hurt'; u.poseT = 20; Audio.sfx('ink_jet'); const wv = fx(999, () => { const k = clamp(wv.t / 16, 0, 1), c = PJ([lerp(u.wx, t.wx, k), lerp(u.wy, t.wy, k), 0]); g.fillStyle = '#1e1a2c'; g.beginPath(); g.ellipse(c[0], c[1], (14 + k * 8) * c[2], 6 * c[2], 0, 0, 6.29); g.fill(); g.fillStyle = '#4a4460'; g.fillRect(Math.round(c[0] - 8 * c[2]), Math.round(c[1] - 8 * c[2]), Math.round(16 * c[2]), 2); }, true);
