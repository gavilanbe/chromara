...
function* atkPincel(u, t) { // el pincel se moja en Añil (se agita), pinta una voluta en el aire que sale disparada como latigazo de tinta y salpica gotas que ruedan por el suelo
  ...
  // el pincel gira sobre la mano, al costado de Añil: lo sacude arriba y abajo para cargarlo
  const pf = fx(999, () => { const c = PJ(above(u, .5)), sc = c[2], a = -2.05 + st.dip * .03, L = 30 * sc; drawProp(img, c[0] + 7 * sc + Math.cos(a) * L, c[1] + Math.sin(a) * L, a, 1, P.tip[0], P.tip[1], .7 * sc); });
  ...
  u.pose = 'attack';
  yield* toolStroke({kind:'pincel',color:col,pts,w:4,col,frames:18,fac:-1,life:52,scale:.7,hitAt:1,
...
function* techSalpicon(u, targets, tech, col) { // Añil salta, el pincel gira como aspa y llueven gotas azules en arco sobre todos, con sombras que anuncian dónde cae cada una
  ...
  const pf = fx(999, () => { const c = PJ(above(u, 1.2)); drawProp(img, c[0], c[1], st.a, 1, 24, 8, .75 * c[2]); });
...
function* techTachon(u, t, tech, col) { // Carmín salta agarrada al lápiz como a una pértiga, lo clava de punta delante del enemigo y lo arrastra corriendo a través de él: queda tachado y lento
  camFocus(u.wx, u.wy, { dist: 70, turn: .3, h: 40, ease: .1 }); yield* anticipate(u, 14); camFocus(t.wx, t.wy, { dist: 82, turn: -.3, h: 52, pitch: .68, headroom: 44 }); u.pose = 'attack'; yield* whop(u, ...infront(u, t, t.def.w * .8 + 26), 10, 14); yield* wait(4);
  const img = propSprite('lapiz', C(col)), P = PROP.lapiz, [sx, sy] = towards(u, t, .4), st = { x: u.wx, y: u.wy, wz: 0, a: 1.25, alpha: 1 }, gh = propGhosts(4), sh = shadowFx(sx, sy, 3, 999), x0 = u.wx, y0 = u.wy;
  // punta abajo: el cuerpo del lápiz sube desde la punta y Carmín va agarrada a él
  const pf = fx(999, () => { const c = PJ([st.x, st.y, st.wz]); gh.push(img, c[0], c[1], st.a, 1, P.tip[0], P.tip[1], 1.2 * c[2]); gh.draw(); drawProp(img, c[0], c[1], st.a, 1, P.tip[0], P.tip[1], 1.2 * c[2], st.alpha); });
  Audio.sfx('charge', { semi: 0 }); Audio.sfx('fwip', { when: .1 });
  for (let i = 1; i <= 10; i++) { const k = i / 10, e = Math.sin(k * Math.PI / 2); u.wz = e * 48; u.wx = lerp(x0, sx, k * .35); u.wy = lerp(y0, sy, k * .35); st.x = lerp(u.wx, sx, .4); st.y = lerp(u.wy, sy, .4); st.wz = u.wz - 6; st.a = 1.25 - k * .3;
    u.gesture = Prefs.shake ? { sx: .86, sy: 1.18 } : null; sh.draw = () => { const c = project(sx, sy, 0); if (c) shadow(c[0], c[1], Math.round((3 + k * 3) * c[2])); }; yield; }
  for (let i = 0; i < 3; i++) { u.gesture = null; u.wz += .5; st.wz += .5; yield; } // un instante suspendida en lo alto
  const hx = u.wx, hy = u.wy, hz = u.wz, pz = st.wz;
  for (let i = 1; i <= 5; i++) { const k = i / 5, e = k * k; st.x = lerp(st.x, sx, e); st.y = lerp(st.y, sy, e); st.wz = lerp(pz, 0, e); st.a = .95 + .3 * e; u.wx = lerp(hx, sx + (x0 - sx) * .45, e); u.wy = lerp(hy, sy + (y0 - sy) * .45, e); u.wz = lerp(hz, 6, e); u.gesture = Prefs.shake ? { sx: .84, sy: 1.22 } : null; yield; }
  sh.dur = 0; st.x = sx; st.y = sy; st.wz = 0; u.wz = 0; u.gesture = Prefs.shake ? { sx: 1.3, sy: .72 } : null;
  B.shake = 6; B.hitstop = 5; Audio.sfx('impact_sub', { vol: .7 }); burst(sx, sy, 2, GRAPHITE, 12, 1.8, 20, .1); burst(sx, sy, 2, '#e8cf9a', 6, 1.4, 16, .1); mark({ kind: 'blob', p: [sx, sy, 0], w: 6, col: GRAPHITE2, seed: 4, life: 140, under: true });
  for (let j = 0; j < 4; j++) { const a = j * 1.57 + .4; mark({ kind: 'path', pts: [[sx, sy, 0], [sx + Math.cos(a) * 9, sy + Math.sin(a) * 4, 0]], w: 1, col: GRAPHITE2, grow: 3, life: 140, under: true }); } // grietas donde se clava
  yield* wait(5); u.gesture = null;
  // arrastre pesado: Carmín corre a través del enemigo tirando del lápiz clavado, que se inclina hacia atrás y raya el suelo
  Audio.sfx('scratch_long'); const to = fwdOf(t, 26), pts = [[sx, sy, 2], [t.wx, t.wy, t.def.h * .35], [to[0], to[1], 2]], ox = u.wx - sx, oy = u.wy - sy;
  mark({ kind: 'path', pts, w: 7, col: GRAPHITE, grow: 14, life: 160, jit: 1 }); mark({ kind: 'path', pts: pts.map(q => [q[0] + 1, q[1] + 1, q[2]]), w: 3, col: GRAPHITE2, grow: 14, life: 160 });
  const run = ghostsOf(u);
  for (let i = 1; i <= 14; i++) { const k = i / 14, q = pointAt(pts, k); st.x = q[0]; st.y = q[1]; st.wz = q[2]; st.a = 1.25 + .45 * Math.sin(k * Math.PI); u.wx = q[0] + ox * (1 - k * 1.6); u.wy = q[1] + oy * (1 - k * 1.6); u.wz = Math.abs(Math.sin(i * .9)) * 3; if (i % 2) run.add();
    if (i % 3 === 0) { B.shake = 3; burst(q[0], q[1], q[2], GRAPHITE, 4, 1, 14, .1); } if (i === 7) { B.slowmo = 10; damage(t, baseDmg(u.atk * statusMult(u, 'tiznado'), t.dfn, tech.power), col, u.name); applyStatus(t, 'lento', 3); goop(t, GRAPHITE, 60); } yield; }
  run.end(); u.wz = 0;
  // lo arranca de un tirón y el lápiz sale girando hasta desvanecerse
  Audio.sfx('fwip', { semi: 5 }); for (let i = 1; i <= 8; i++) { st.wz += 5; st.a += .35; st.alpha = 1 - i / 9; yield; }
  pf.dur = 0; yield* wait(14); yield* returnHome(u, 14); yield* wait(8); camReset();
}
function* techManchurron(u, t, tech, col) { // Carmín carga el pincel hasta que cuelga una gota enorme que tiembla y gotea; lo echa atrás, lo sacude y la gota revienta en el enemigo
  camFocus(u.wx, u.wy, { dist: 70, turn: .35, h: 40, ease: .1, headroom: 46 }); u.pose = 'charge';
  const CO = C(col), rp = ramp(CO), img = propSprite('pincel', CO), P = PROP.pincel, st = { load: 0, a: -2.05, wob: 0, alpha: 1 }, blob = { on: false, wx: 0, wy: 0, wz: 0, spin: 0 };
  // el pincel gira sobre la mano (un punto al costado de Carmín): echarlo atrás y sacudirlo es un giro de muñeca
  const tip = () => { const c = PJ(above(u, .5)), s = c[2], L = 30 * s; return [c[0] + 7 * s - Math.cos(st.a) * -L, c[1] - Math.sin(st.a) * -L, s]; };
  // La gota de pintura: contorno, cuerpo, sombra abajo, brillo húmedo y un hilo que gotea; tiembla con su peso.
  const drawLoad = (x, y, r, s, wob, hang) => { const sy = 1 + wob * .12, sx = 1 - wob * .08; g.fillStyle = rp.out; g.beginPath(); g.ellipse(x, y + r * hang, r * sx + 1, r * sy + 1, 0, 0, 6.29); g.fill(); g.fillStyle = rp.base; g.beginPath(); g.ellipse(x, y + r * hang, r * sx, r * sy, 0, 0, 6.29); g.fill();
    g.fillStyle = rp.sh; g.beginPath(); g.ellipse(x + r * .15, y + r * hang + r * .45, r * .7 * sx, r * .4 * sy, 0, 0, 6.29); g.fill(); g.fillStyle = rp.hi; g.fillRect(Math.round(x - r * .5), Math.round(y + r * hang - r * .55), Math.max(2, Math.round(r * .4)), Math.max(1, Math.round(r * .18))); g.fillStyle = '#fff8e6'; g.fillRect(Math.round(x - r * .45), Math.round(y + r * hang - r * .5), 1, 1); };
  const pf = fx(999, () => { const [x, y, s] = tip(); drawProp(img, x, y, st.a, 1, P.tip[0], P.tip[1], .7 * s, st.alpha); if (!blob.on && st.load > 0) drawLoad(x, y, (2 + st.load * 8) * s, s, st.wob, .7); });
  sparkle(...above(u, 1.2), rp.hi); Audio.sfx('squeeze');
  for (let i = 0; i < 30; i++) { st.load = i / 30; st.wob = Math.sin(i * .7) * st.load; u.wz = Math.sin(i * .9) * 1.2; u.sqz = st.load * .6;
    if (i % 2 === 0) B.particles.push({ wx: u.wx + R(-8, 8), wy: u.wy + R(-4, 4), wz: R(2, u.def.h * .6), tx: u.wx, ty: u.wy, tz: u.def.h + 14, col: CO, t: 0, life: 10, dur: 10, arc: 6, stream: true });
    if (i > 14 && i % 5 === 0) drips(u.wx + 4, u.wy, u.def.h + 6, CO, 1); if (i === 20) Audio.sfx('plop', { semi: 2 }); yield; }
  u.sqz = 0; u.wz = 0; u.pose = 'attack';
  // echa el pincel atrás (la gota se estira por la inercia) y lo sacude hacia delante: la gota sale disparada
  Audio.sfx('charge', { semi: 0, vol: .45 }); for (let i = 1; i <= 7; i++) { st.a = lerp(-2.05, -2.7, i / 7); st.wob = -1.2 * i / 7; u.gesture = Prefs.shake ? { sx: 1 + .08 * i / 7, sy: 1 - .08 * i / 7 } : null; yield; }
  Audio.sfx('whip'); camFocus(t.wx, t.wy, { dist: 78, turn: -.3, h: 44 });
  for (let i = 1; i <= 3; i++) { st.a = lerp(-2.7, -1.1, i / 3); u.gesture = Prefs.shake ? { sx: .9, sy: 1.1 } : null; yield; }
  const from = [u.wx + 4, u.wy, u.def.h + 12]; blob.on = true; u.gesture = null;
  const trail = [], bf = fx(999, () => { trail.forEach((p, i) => { const c = PJ(p), r = Math.max(1, (2 + i) * c[2] * .5); g.fillStyle = i % 2 ? rp.base : rp.hi; g.fillRect(Math.round(c[0] - r / 2), Math.round(c[1] - r / 2), Math.ceil(r), Math.ceil(r)); });
    const c = PJ([blob.wx, blob.wy, blob.wz]); drawLoad(c[0], c[1], 10 * c[2], c[2], Math.sin(blob.spin) * 1.4, 0); });
  for (let i = 1; i <= 14; i++) { const k = i / 14, e = k * k; blob.wx = lerp(from[0], t.wx, e); blob.wy = lerp(from[1], t.wy, e); blob.wz = lerp(from[2], t.def.h * .5, e) + Math.sin(k * Math.PI) * 26; blob.spin += .9;
    trail.push([blob.wx, blob.wy, blob.wz]); if (trail.length > 5) trail.shift(); st.alpha = Math.max(0, 1 - i / 8); yield; }
  bf.dur = 0; pf.dur = 0; Audio.sfx('splash'); B.shake = 7; B.hitstop = 8; B.slowmo = 12;
  damage(t, baseDmg(u.atk * statusMult(u, 'tiznado'), t.dfn, tech.power), col, u.name); goop(t, CO, 160); drips(t.wx, t.wy, t.def.h * .8, CO, 12); lensSplatter(CO, 8, 5);
  // el reventón: rayos de pintura por el suelo, un pegote que chorrea por el enemigo y el charco debajo
  for (let j = 0; j < 9; j++) { const a = j / 9 * 6.28 + .2, L = t.def.w * (.7 + (j % 3) * .25), ex = t.wx + Math.cos(a) * L, ey = t.wy + Math.sin(a) * L * .45; mark({ kind: 'path', pts: [[t.wx, t.wy, 0], [ex, ey, 0]], w: 3 - (j % 2), col: CO, grow: 5, life: 170, under: true }); mark({ kind: 'blob', p: [ex, ey, 0], w: 3 + (j % 3), col: CO, seed: 20 + j, life: 170, grow: 4, under: true }); }
  mark({ kind: 'blob', p: [t.wx + 2, t.wy, t.def.h * .98], w: t.def.w * .26, col: CO, seed: 9, life: 150, grow: 3 }); mark({ kind: 'pool', p: [t.wx, t.wy + 2, 0], w: t.def.w * .9, col: rp.sh, grow: 16, life: 180, under: true });
  burst(t.wx, t.wy, t.def.h * .6, CO, 18, 2.4, 26, .1); burst(t.wx, t.wy, t.def.h * .6, rp.hi, 6, 1.6, 20, .08);
...
function* techAguada(u, targets, tech, col) { // Añil moja la brocha en sí misma y barre el campo con una banda de color diluido que empapa a todos
  ...
  const pf = fx(999, () => { const c = PJ(above(u, .5)), sc = c[2], a = -2.0 + st.dip * .03, L = 34 * sc; drawProp(img, c[0] + 7 * sc + Math.cos(a) * L, c[1] + Math.sin(a) * L, a, 1, P.tip[0], P.tip[1], .6 * sc); });
  ...
  // la aguada: una banda de agua coloreada con el borde de arriba ondulado y brillante, y la brocha barriendo al frente
  const bro = propSprite('brocha', C(col)), wrp = ramp(C(col)), band = fx(999, () => { const k = clamp(band.t / 24, 0, 1), k0 = Math.max(0, k - .4), n = 14, top = [], bot = [];
    for (let i = 0; i <= n; i++) { const q = lerp(k0, k, i / n), p = PJ([lerp(L[0], Rr[0], q), lerp(L[1], Rr[1], q), 0]), hgt = (38 + Math.sin(q * 19 + band.t * .3) * 4) * p[2] * Math.min(1, (i / n) * 3); top.push([p[0], p[1] - hgt]); bot.push([p[0], p[1] + 3]); }
    const poly = [...top, ...bot.reverse()]; g.globalAlpha = .38; g.fillStyle = C(col); g.beginPath(); poly.forEach((p, i) => i ? g.lineTo(p[0], p[1]) : g.moveTo(p[0], p[1])); g.closePath(); g.fill();
    g.globalAlpha = .5; for (let i = 1; i < top.length; i++) { pstroke(top[i - 1][0], top[i - 1][1] + 3, top[i][0], top[i][1] + 3, 1, wrp.hi, 1, 0, false); }
    g.globalAlpha = 1; if (k < 1) { const f = top[top.length - 1]; drawProp(bro, f[0], f[1] + 20, -1.75, 1, PROP.brocha.tip[0], PROP.brocha.tip[1], .7 * PJ([Rr[0], Rr[1], 0])[2]); } });
...
// Nivel 2 de la Llamarada, Fénix («la página arde»): rojo y amarillo se funden en naranja y la gota sube al cielo. Ámbar esboza a lápiz un fénix a su
...
function* techLlamaradaFenix(users, targets, tech, col) { // nivel 2 de la Llamarada: Fénix, se compra a Sepia
  const red = users.find(u => u.id === 'carmin') || users[0], yel = users.find(u => u.id === 'ambar') || users[1] || users[0], mid = centroid(users), ec = enemyC();
  const OR = C('naranja'), YE = C('amarillo'), RE = C('rojo'), dmg = t => damage(t, baseDmg(users.reduce((s, u) => s + u.atk * statusMult(u, 'tiznado'), 0) / users.length, t.dfn, tech.power), col, tech.name);
...
// encendiendo a cada enemigo cuando su frente de llamas lo alcanza, envuelto en fuego por delante y por detrás, con humo, ascuas y el suelo chamuscado.
function* techLlamarada(users, targets, tech, col) {
  ...
  const bro = propSprite('brocha', RE), lap = propSprite('lapiz', YE), st = { k: 0, fire: 0, hold: 1, scrape: 0, swing: 0, lift: 0, star: 0 };
  ...
  const gh = propGhosts(5), sparks = [], srnd = seeded(31), arc = [];
  const pf = fx(999, () => {
    ...
    for (const sp of sparks) { if (sp.t < 0 || sp.t > sp.life) continue; const q = sp.t / sp.life, x = sp.x + sp.vx * sp.t, y = sp.y + sp.vy * sp.t + .12 * sp.t * sp.t; g.fillStyle = q < .4 ? '#fff3c0' : q < .7 ? YE : OR; g.fillRect(Math.round(x), Math.round(y), sp.big ? 2 : 1, sp.big ? 2 : 1); if (q < .5) g.fillRect(Math.round(x - sp.vx), Math.round(y - sp.vy), 1, 1); }
    // arden todas las cerdas: brasa a lo largo de la cabeza de la brocha y una lengua por mechón, la de la punta la mayor
    if (st.fire > 0) { const [tx, ty, s] = tipAt(); g.globalAlpha = .28 * st.fire * Math.max(.5, Prefs.flash); pixDisc(tx - 6 * s, ty, (10 + st.fire * 16) * s, OR); g.globalAlpha = 1;
      for (let j = 0; j < 5; j++) { const f = clamp(st.fire * 1.6 - j * .15, 0, 1); if (f <= 0) continue; const [bx, by] = local(b, 40 + j * 4.4, 4 + j * 1.2); g.fillStyle = (pf.t + j) % 4 < 2 ? '#ffb347' : '#fff3c0'; g.fillRect(Math.round(bx), Math.round(by), 2, 1); drawFire(bx, by + 3 * s, (2 + f * 3 + (j === 4 ? 3 : 0)) * s, (6 + f * (j === 4 ? 26 : 14)) * s, pf.t + j * 4, j + 1, .95); } }
    // el bateo deja un arco de fuego detrás de la brocha
    arc.forEach((p, i) => { if (i) pstroke(arc[i - 1][0], arc[i - 1][1], p[0], p[1], Math.max(1, (2 + i * 1.4) * b.s), i > arc.length - 3 ? '#fff3c0' : i % 2 ? OR : YE, 1, 0, false); });
    if (st.star > 0) { const [tx, ty, s] = tipAt(), r = (6 + st.star * 10) * s; g.fillStyle = '#fff3c0'; g.fillRect(Math.round(tx - r), Math.round(ty), Math.round(r * 2), 1); g.fillRect(Math.round(tx), Math.round(ty - r), 1, Math.round(r * 2)); g.fillRect(Math.round(tx - r * .4), Math.round(ty - 1), Math.round(r * .8), 3); g.fillRect(Math.round(tx - 1), Math.round(ty - r * .4), 3, Math.round(r * .8)); }
  });
  pf.tick = () => { for (const sp of sparks) sp.t++; while (sparks.length && sparks[0].t > sparks[0].life) sparks.shift(); };
  const spark = n => { if (!Prefs.shake) return; const b = brushAt(), [px, py] = local(b, 37 + st.k * 22, -7 + st.k * 5); for (let j = 0; j < n; j++) sparks.push({ x: px, y: py, vx: (srnd() - .5) * 4, vy: -1 - srnd() * 2.8, t: 0, life: 10 + srnd() * 10, big: srnd() < .3 }); };
  ...
  for (let r = 0; r < 3; r++) {
    ...
    smokeColumn(sparkAt[0] + 4, sparkAt[1], sparkAt[2] - 6, 2, 8, 4, 11 + r); // bocanada de fricción al final de cada pasada
  ...
  for (let i = 1; i <= 5; i++) { st.swing = -.55 + 2.2 * (i / 5) ** 1.6; st.lift = 4 - 6 * i / 5; red.gesture = Prefs.shake ? { sx: 1.12 - .12 * i / 5, sy: .88 + .12 * i / 5 } : null; if (Prefs.shake) { const [tx, ty] = tipAt(); arc.push([tx, ty]); } if (i === 3) { st.fire = 0; red.wz = 6; st.star = 1; Audio.sfx('impact_sub', { vol: .5 }); } yield; }
  red.gesture = null; red.wz = 0; B.hitstop = 4;
  ...
  const bf = fx(999, () => {
    ...
    drawFire(c[0], c[1] + 9 * c[2], 13 * c[2], 36 * c[2], bf.t, 2); pixDisc(c[0], c[1] - 2 * c[2], Math.max(2, 7 * c[2]), C('amarillo'), null, '#ffffff'); pixDisc(c[0] - 1, c[1] - 3 * c[2], Math.max(1, 4 * c[2]), '#fff3c0');
  });
  for (let i = 1; i <= 22; i++) { const k = i / 22; st.star = Math.max(0, st.star - .25); if (arc.length && i % 2) arc.shift(); if (i <= 8) { st.swing = lerp(1.65, 0, i / 8); st.lift = lerp(-2, 0, i / 8); } else st.hold = 0; if (i % 2) ball.trail.unshift([ball.wx, ball.wy, ball.wz]); if (ball.trail.length > 7) ball.trail.pop(); ball.wx = lerp(red.wx, ec[0], k); ball.wy = lerp(red.wy, ec[1], k); ball.wz = red.def.h + 8 + Math.sin(k * Math.PI) * 30 - k * 20;
    if (i % 4 === 2) { const dx = ball.wx, dy = ball.wy; B.particles.push({ wx: dx, wy: dy, wz: ball.wz, vx: 0, vy: 0, vz: -.5, g: .12, col: OR, t: 0, life: 16, size: 2 }); afterFx(8, () => { mark({ kind: 'blob', p: [dx, dy, 0], w: 3, col: '#3a2a22', seed: i, grow: 3, life: 200, under: true }); flames(dx, dy, 1, 2); }); } // gotas ardiendo que chamuscan el suelo
  ...
  // Frente de llamas: un anillo de lenguas de fuego que se abre por el suelo desde el impacto; enciende a cada enemigo al alcanzarlo.
  const front = { r: 0, a: 1 }, ringB = fx(999, () => { if (front.r <= 0 || front.a <= 0) return; const n = 30; for (let j = 0; j < n; j++) { const an = j / n * 6.283, back = Math.sin(an) < 0; if (!back) continue; const c = PJ([ec[0] + Math.cos(an) * front.r, ec[1] + Math.sin(an) * front.r, 0]); drawFire(c[0], c[1], 4 * c[2], (10 + 4 * Math.sin(ringB.t * .5 + j)) * c[2], ringB.t + j * 2, j, front.a); } }, true),
    ringF = fx(999, () => { if (front.r <= 0 || front.a <= 0) return; const n = 30; for (let j = 0; j < n; j++) { const an = j / n * 6.283; if (Math.sin(an) < 0) continue; const c = PJ([ec[0] + Math.cos(an) * front.r, ec[1] + Math.sin(an) * front.r, 0]); drawFire(c[0], c[1], 4 * c[2], (10 + 4 * Math.sin(ringF.t * .5 + j)) * c[2], ringF.t + j * 2, j + 40, front.a); } });
  ...
  const pillars = order.map((t, j) => ({ t, seed: j * 3 + 1, k: 0, at: 99, lit: false, d: Math.hypot(t.wx - ec[0], t.wy - ec[1]) })), reach = Math.max(40, ...pillars.map(p => p.d + 14));
  const fireOf = (p, front) => { const c = PJ([p.t.wx, p.t.wy, 0]), s = c[2] * (p.t.boss ? 1.35 : 1.6), w = p.t.def.w * (front ? .38 : .7) * s * p.k, h = p.t.def.h * (front ? 1.2 : 2.3) * s * p.k; drawFire(c[0] + (front ? 2 : 0), c[1] + (front ? 3 : 1), w, h, pilB.t + p.seed * 5, p.seed + (front ? 20 : 0), front ? .8 : 1); };
  const pilB = fx(999, () => { for (const p of pillars) if (p.k > 0) fireOf(p, false); }, true), pilF = fx(999, () => { for (const p of pillars) if (p.k > 0) fireOf(p, true); });
  for (let i = 0; i < 50; i++) {
    front.r = reach * Math.min(1, (1 - (1 - i / 26) ** 2)); front.a = i < 30 ? 1 : Math.max(0, (40 - i) / 10);
    for (const p of pillars) {
      if (!p.lit && front.r >= p.d) p.at = i;
  ...
  pilB.dur = 0; pilF.dur = 0; ringB.dur = 0; ringF.dur = 0;
...
function pixDisc(x, y, r, col, out, hi, ry = r, dither = 0) {
  if (!(r > 0) || !(ry > 0)) return;
  const rows = (rr, rry, c) => { g.fillStyle = c; const R2 = Math.ceil(rry); for (let dy = -R2; dy <= R2; dy++) { const q = 1 - (dy / rry) ** 2; if (q < 0) continue; const w = rr * Math.sqrt(q), yy = Math.round(y + dy), xa = Math.round(x - w), xb = Math.round(x + w); if (!dither) g.fillRect(xa, yy, Math.max(1, xb - xa), 1); else for (let px = xa + ((xa + yy) & 1); px < xb; px += 2) g.fillRect(px, yy, 1, 1); } };
  if (out) rows(r + 1, ry + 1, out); rows(r, ry, col);
  if (hi && r >= 3) { g.fillStyle = hi; g.fillRect(Math.round(x - r * .5), Math.round(y - ry * .6), Math.max(1, Math.round(r * .45)), 1); g.fillRect(Math.round(x - r * .65), Math.round(y - ry * .4), 1, Math.max(1, Math.round(ry * .3))); }
}
// Anillo de píxel: puntos sueltos sobre una elipse (ondas en el agua).
function pixRing(x, y, rx, ry, col) { if (!(rx >= 1)) return; g.fillStyle = col; const n = Math.max(8, Math.round(rx * 4)); for (let i = 0; i < n; i++) { const a = i / n * 6.28; g.fillRect(Math.round(x + Math.cos(a) * rx), Math.round(y + Math.sin(a) * ry), 1, 1); } }
...
// ---- Nivel 2 del Brote: Selva. La gota verde se hunde en el claro; Ámbar traza a lápiz un tronco que sube hasta el cielo y
// abre ramas, y Añil le pinta la copa a pinceladas redondas. Las raíces corren bajo tierra hasta cada enemigo, revientan el suelo
// bajo sus pies y lo alzan envuelto; la copa florece y suelta frutos amarillos y azules que caen sobre el grupo y lo curan.
function* techSelva(users, targets, tech, col) {
  const yel = users.find(u => u.id === 'ambar') || users[0], blu = users.find(u => u.id === 'anil') || users[1] || users[0], mid = centroid(users), ec = enemyC(), pc = partyC();
  const GR = C('verde'), YE = C('amarillo'), BL = C('azul'), LP = leafPalette(), YR = ramp(YE), BR = ramp(BL);
  const dmg = t => damage(t, baseDmg(users.reduce((s, u) => s + u.atk * statusMult(u, 'tiznado'), 0) / users.length, t.dfn, tech.power), col, tech.name);
  camFocus(mid[0], mid[1], { dist: 96, turn: .35, h: 52, pitch: .55, hy: 84, ease: .1 }); users.forEach(u => u.pose = 'charge');
  const { of, orb } = yield* fuseAt(users, mid[0], mid[1], 22, GR); yield* wait(4);
  // 1) la gota se hunde en el claro entre los dos bandos
  const sp = [lerp(ec[0], pc[0], .5), lerp(ec[1], pc[1], .5)], TOP = 74;
  camFocus(sp[0], sp[1], { dist: 92, turn: .25, h: 52, pitch: .46, f: 170, hy: 100, ease: .08, subjects: users, points: [[sp[0], sp[1], TOP, 26]] });
  Audio.sfx('fwip'); const r0 = orb.r;
  for (let i = 1; i <= 16; i++) { const k = i / 16; orb.wx = lerp(mid[0], sp[0], k); orb.wy = lerp(mid[1], sp[1], k); orb.wz = 22 + Math.sin(k * Math.PI) * 36 - k * 22; orb.r = lerp(r0, 4, k); yield; }
  of.dur = 0; Audio.sfx('plop', { semi: -5 }); B.shake = 3; burst(sp[0], sp[1], 2, '#8a6a4a', 12, 1.6, 20, .1); mark({ kind: 'pool', p: [sp[0], sp[1] + 1, 0], w: 12, col: '#5a4630', grow: 6, life: 480, under: true });
  // 2) el tronco: Ámbar lo traza desde la tierra hasta el cielo; se ensancha abajo y abre cuatro ramas
  const tree = { trunk: 0, branches: 0, canopy: 0, bloom: 0, alpha: 1, sway: 0 }, lap = propSprite('lapiz', YE), pin = propSprite('pincel', BL), tool = { pencil: 0, brush: 0, bx: 0, by: 0 };
  const trunkPts = Array.from({ length: 9 }, (_, i) => { const k = i / 8; return [sp[0] + Math.sin(k * 3) * 3, sp[1], k * TOP]; });
  const branchPts = [[.55, -1, 26, 16], [.68, 1, 24, 12], [.8, -1, 18, 10], [.86, 1, 16, 8]].map(([k, dir, len, rise]) => { const b = pointAt(trunkPts, k); return [b, [b[0] + B.axis.rx * dir * len * .5, b[1] + B.axis.ry * dir * len * .5, b[2] + rise * .6], [b[0] + B.axis.rx * dir * len, b[1] + B.axis.ry * dir * len, b[2] + rise]]; });
  const blobs = [[0, 0, 0, 22], [-18, 0, -6, 15], [18, 0, -4, 16], [-9, 0, 12, 14], [10, 0, 11, 14], [-24, 0, 8, 11], [24, 0, 9, 11], [0, 0, 20, 12]]; // [dx lateral, _, dz, radio]
  const bark = { out: '#241e32', base: '#7a5230', hi: '#a8784a', sh: '#5a3a22' };
  const tf = fx(999, () => {
    g.globalAlpha = tree.alpha;
    if (tree.trunk > 0) { drawVine(trunkPts, tree.trunk, 9, bark, 2, tree.alpha); drawVine(trunkPts.slice(0, 4), Math.min(1, tree.trunk * 3), 13, bark, 3, tree.alpha); }
    branchPts.forEach((bp, i) => { const k = clamp(tree.branches * 4 - i, 0, 1); if (k > 0) drawVine(bp, k, 4, bark, 5 + i, tree.alpha); });
    // la copa: pinceladas redondas de verde con sombra azulada y brillo amarillo, y flores al final
    const top = PJ([sp[0], sp[1], TOP - 4]), s = top[2];
    blobs.forEach(([dx, , dz, r], i) => { const k = clamp(tree.canopy * blobs.length - i, 0, 1); if (k <= 0) return; const x = top[0] + dx * s + Math.sin(tf.t * .05 + i) * tree.sway, y = top[1] - dz * s, rr = r * s * (.5 + .5 * k);
      pixDisc(x + 2, y + 3, rr, LP.sh, LP.out); pixDisc(x, y, rr * .92, LP.base, null, LP.hi); pixDisc(x - rr * .3, y - rr * .35, rr * .35, LP.hi); });
    if (tree.bloom > 0) blobs.forEach(([dx, , dz, r], i) => { if (i % 2) return; const x = top[0] + (dx + (i % 3 - 1) * 5) * s, y = top[1] - (dz + r * .4) * s; pixFlower(x, y, tree.bloom, s * .8, tf.t + i * 7, i); });
    if (tool.pencil > 0) { const tip = PJ(pointAt(trunkPts, tree.trunk)); drawProp(lap, tip[0] + 2, tip[1], -1.05, 1, PROP.lapiz.tip[0], PROP.lapiz.tip[1], .6 * tip[2], tool.pencil); }
    if (tool.brush > 0) drawProp(pin, tool.bx, tool.by, -2.1, 1, PROP.pincel.tip[0], PROP.pincel.tip[1], .6 * s, tool.brush);
    g.globalAlpha = 1;
  });
  yel.pose = 'attack'; Audio.sfx('scratch_long'); Audio.sfx('grow', { when: .2 });
  for (let i = 1; i <= 26; i++) { tree.trunk = 1 - (1 - i / 26) ** 2; tool.pencil = Math.min(1, i / 4); yel.wz = Math.abs(Math.sin(i * .6)) * 3; if (i % 3 === 0) B.particles.push({ wx: sp[0] + R(-4, 4), wy: sp[1], wz: tree.trunk * TOP, vx: R(-.3, .3), vy: 0, vz: -R(.2, .6), g: .05, col: '#8a6a4a', t: 0, life: 16, size: 1 }); if (i % 6 === 0) { B.shake = 1; Audio.sfx('crumbs', { vol: .4 }); } yield; }
  Audio.sfx('leaves', { vol: .6 }); for (let i = 1; i <= 12; i++) { tree.branches = i / 12; tool.pencil = 1 - i / 12; yield; } yel.wz = 0; yel.pose = 'idle';
  // 3) la copa: Añil pinta pinceladas redondas, una tras otra
  blu.pose = 'attack'; Audio.sfx('brush_sweep');
  for (let i = 1; i <= 24; i++) { tree.canopy = i / 24; const n = Math.min(blobs.length - 1, Math.floor(tree.canopy * blobs.length)), top = PJ([sp[0], sp[1], TOP - 4]), [dx, , dz] = blobs[n]; tool.brush = Math.min(1, i / 4); tool.bx = top[0] + dx * top[2] + 6; tool.by = top[1] - dz * top[2] - 4 + Math.sin(i * 1.4) * 3; blu.wz = Math.abs(Math.sin(i * .7)) * 3; if (i % 4 === 0) Audio.sfx('brush_big', { vol: .4 }); yield; }
  for (let i = 1; i <= 5; i++) { tool.brush = 1 - i / 5; yield; } blu.wz = 0; blu.pose = 'idle'; tree.sway = 1.2;
  // 4) raíces: corren bajo tierra desde la base hasta cada enemigo, revientan el suelo y lo alzan enroscado
  camFocus(ec[0], ec[1], { dist: 118, turn: .9, h: 56, pitch: .54, ease: .08, subjects: targets, headroom: 26, points: [[sp[0], sp[1], 0, 10]] }); // de lado: el tronco queda al borde y las raíces cruzan el plano
  const order = [...targets].sort((a, b) => Math.hypot(a.wx - sp[0], a.wy - sp[1]) - Math.hypot(b.wx - sp[0], b.wy - sp[1]));
  const roots = order.map((t, j) => { const n = 8, ground = [], climb = []; for (let i = 0; i <= n; i++) { const k = i / n, bow = Math.sin(k * Math.PI); ground.push([lerp(sp[0], t.wx, k) + Math.sin(k * 7 + j) * 5 * bow, lerp(sp[1], t.wy, k) + Math.cos(k * 5 + j) * 3 * bow, 0]); }
    const es = t.boss ? 1.35 : 1.6; for (let i = 0; i <= 10; i++) { const a = i * 1.1 + j, k = i / 10; climb.push([t.wx + Math.cos(a) * t.def.w * .45 * es, t.wy + Math.sin(a) * 5, k * t.def.h * .9 * es]); }
    return { t, j, ground, climb, gk: 0, ck: 0, at: j * 6, lift: 0, hit: false }; });
  const rg = fx(999, () => { for (const r of roots) if (r.gk > 0) drawVine(r.ground, r.gk, 3.4, bark, r.j + 11, tree.alpha); }, true);
  const rc = fx(999, () => { for (const r of roots) if (r.ck > 0) drawVine(r.climb.map(p => [p[0], p[1], p[2] + r.lift]), r.ck, 3, LP, r.j + 21, tree.alpha); });
  for (let i = 0; i < 58; i++) {
    for (const r of roots) { const a = i - r.at; if (a < 0) continue; r.gk = clamp(a / 14, 0, 1);
      if (a === 14) { Audio.sfx('crumbs'); Audio.sfx('leaves', { vol: .6 }); burst(r.t.wx, r.t.wy, 2, '#8a6a4a', 14, 2, 22, .12); mark({ kind: 'blob', p: [r.t.wx, r.t.wy, 0], w: r.t.def.w * .55, col: '#5a4630', grow: 6, life: 300, seed: r.j + 3, under: true }); B.shake = Math.max(B.shake, 4); }
      if (a >= 14) { const q = clamp((a - 14) / 12, 0, 1); r.ck = q; r.lift = Math.sin(Math.min(1, q * 1.2) * Math.PI * .5) * 16 * (a > 40 ? Math.max(0, (52 - a) / 12) : 1); r.t.wz = r.lift; r.t.sqz = q; if (!r.hit && q >= .7) { r.hit = true; dmg(r.t); goop(r.t, GR, 100); impactFrame(r.t, GR, .9); } } }
    yield;
  }
  ...
  for (let i = 0; i < 44; i++) { for (const f of fruits) { if (f.done) continue; f.k += 1 / 18; if (f.k >= 1) { f.done = true; Audio.sfx('plop', { semi: 5 }); burst(f.p.wx, f.p.wy, f.p.def.h, f.col, 6, 1.2, 16, .06); if (!healed.has(f.p)) { healed.add(f.p); heal(f.p, Math.round(f.p.maxhp * tech.heal)); f.p.uiHeal = 20; sparkle(f.p.wx, f.p.wy, f.p.def.h + 8, '#fbe28a'); f.p.pose = 'happy'; } } }
    if (i % 3 === 0) B.particles.push({ wx: sp[0] + R(-26, 26), wy: sp[1] + R(-6, 6), wz: TOP, vx: R(-.4, .4), vy: 0, vz: -R(.1, .3), g: .01, col: i % 2 ? LP.hi : LP.base, t: 0, life: 50, size: 2 }); yield; }
  ff.dur = 0;
  // 6) la selva vuelve a la página: el árbol se desvanece y quedan hojas y la tierra removida
...
// filas) mientras Añil pinta una luna azul con el pincel; luego el pincel arrastra la luna sobre el sol, su sombra tramada barre el
// suelo y donde los dos manchones se solapan nace el violeta. En la totalidad brota la corona, y el disco violeta cae como un tampón
// con estela, se aplasta al chocar, estalla en anillos y deja al enemigo sellado en violeta y lento.
function* techEclipse(users, t, tech, col) {
  const VI = C('violeta'), RE = C('rojo'), BL = C('azul'), VR = ramp(VI), RR = ramp(RE), BR = ramp(BL), PAPER = '#f4f0ea';
  const red = users.find(u => u.id === 'carmin') || users[0], blue = users.find(u => u.id === 'anil') || users[1] || users[0];
  camFocus(t.wx, t.wy, { dist: 100, turn: -.25, h: 62, pitch: .58, f: 160 }); users.forEach(u => u.pose = 'charge');
  const dark = overlay(999, '#0b0912', .72, 40, 14), stars = fx(999, () => { const k = clamp(stars.t / 40, 0, 1), rnd = seeded(77); g.fillStyle = PAPER; for (let i = 0; i < 26; i++) { const sx = rnd() * W, sy = rnd() * 60, tw = (Math.sin(stars.t * .2 + i) + 1) * .5; if (rnd() < k) { g.globalAlpha = k * (.4 + tw * .6); g.fillRect(sx | 0, sy | 0, 1, 1); } } g.globalAlpha = 1; });
  // Los dos manchones viven en la página (pantalla): se pintan por filas y se solapan en violeta.
  const SUNX = W * .38, MOONX = W * .68, SKYY = 34, sun = { x: SUNX, y: SKYY, r: 16, k: 0 }, moon = { x: MOONX, y: SKYY, r: 16, k: 0 }, st = { corona: 0, alpha: 1 };
  const painted = (d, draw) => { if (d.k <= 0) return; g.save(); g.beginPath(); g.rect(d.x - d.r - 2, d.y - d.r - 2, d.r * 2 + 4, (d.r * 2 + 4) * Math.min(1, d.k)); g.clip(); draw(); g.restore(); };
  const discs = fx(999, () => {
    g.globalAlpha = st.alpha;
    // sol rojo con rayos de píxel que laten
    painted(sun, () => { for (let i = 0; i < 12; i++) { const a = i * .52 + discs.t * .03, L = sun.r * (1.4 + .25 * Math.sin(discs.t * .3 + i)); pstroke(sun.x + Math.cos(a) * (sun.r + 2), sun.y + Math.sin(a) * (sun.r + 2), sun.x + Math.cos(a) * L, sun.y + Math.sin(a) * L, 1, RR.hi, 1, 0, false); } pixDisc(sun.x, sun.y, sun.r, RE, RR.out, RR.hi); });
    // luna azul con cráteres; donde tapa al sol, violeta fila a fila
    painted(moon, () => { pixDisc(moon.x, moon.y, moon.r, BL, BR.out, BR.hi); pixDisc(moon.x + moon.r * .3, moon.y + moon.r * .2, Math.max(1, moon.r * .22), BR.sh); pixDisc(moon.x - moon.r * .25, moon.y - moon.r * .4, Math.max(1, moon.r * .16), BR.sh);
      if (Math.hypot(sun.x - moon.x, sun.y - moon.y) < sun.r + moon.r && sun.k > 0) { for (let dy = -Math.ceil(sun.r); dy <= Math.ceil(sun.r); dy++) { if (dy * dy > sun.r * sun.r) continue; const ws = Math.sqrt(sun.r * sun.r - dy * dy), ym = sun.y + dy - moon.y; if (ym * ym > moon.r * moon.r) continue; const wm = Math.sqrt(moon.r * moon.r - ym * ym), x0 = Math.max(sun.x - ws, moon.x - wm), x1 = Math.min(sun.x + ws, moon.x + wm); if (x1 <= x0) continue; g.fillStyle = dy < -sun.r * .55 && x0 < sun.x - sun.r * .2 ? VR.hi : VI; g.fillRect(Math.round(x0), Math.round(sun.y + dy), Math.max(1, Math.round(x1 - x0)), 1); } } });
    // corona violeta y blanca en la totalidad
    if (st.corona > 0) { for (let i = 0; i < 16; i++) { const a = i * .39 + discs.t * .05, L = sun.r * (1.5 + st.corona * 1.1 + .3 * Math.sin(discs.t * .4 + i * 2)); pstroke(sun.x + Math.cos(a) * (sun.r + 2), sun.y + Math.sin(a) * (sun.r + 2), sun.x + Math.cos(a) * L, sun.y + Math.sin(a) * L, i % 4 === 0 ? 2 : 1, i % 2 ? VI : PAPER, 1, 0, false); } }
    g.globalAlpha = 1;
  });
  // Las herramientas suben de cada gota al cielo: la brocha roja barre el sol en tres pasadas, el pincel azul da la vuelta a la luna.
  const bro = propSprite('brocha', RE), pin = propSprite('pincel', BL), tool = { bro: 0, broX: 0, broY: 0, pin: 0, pinX: 0, pinY: 0, pinA: 2.2 }, gh = propGhosts(4);
  const pf = fx(999, () => { if (tool.bro > 0) drawProp(bro, tool.broX, tool.broY, 2.35, 1, PROP.brocha.tip[0], PROP.brocha.tip[1], .8, tool.bro); if (tool.pin > 0) { gh.push(pin, tool.pinX, tool.pinY, tool.pinA, 1, PROP.pincel.tip[0], PROP.pincel.tip[1], .9); gh.draw(); drawProp(pin, tool.pinX, tool.pinY, tool.pinA, 1, PROP.pincel.tip[0], PROP.pincel.tip[1], .9, tool.pin); } });
  Audio.sfx('charge', { semi: 0 }); Audio.sfx('charge', { semi: 7, when: .1 });
  const rs = PJ(above(red, 1.1)), bs = PJ(above(blue, 1.1)); sparkle(...above(red, 1.6), RR.hi); sparkle(...above(blue, 1.6), BR.hi);
  for (let i = 1; i <= 10; i++) { const k = i / 10, e = k * k * (3 - 2 * k); tool.bro = tool.pin = k; tool.broX = lerp(rs[0], sun.x - sun.r, e); tool.broY = lerp(rs[1], sun.y - sun.r * .66, e); tool.pinX = lerp(bs[0], moon.x, e); tool.pinY = lerp(bs[1], moon.y - moon.r, e); tool.pinA = -2.4; yield; }
  Audio.sfx('brush_sweep'); Audio.sfx('brush_hiss', { when: .3, vol: .5 });
  for (let i = 1; i <= 24; i++) { const k = i / 24, pass = Math.min(2, Math.floor(k * 3)), f = k * 3 - pass, dir = pass % 2 ? -1 : 1;
    tool.broX = sun.x + dir * lerp(-sun.r - 4, sun.r + 4, f); tool.broY = sun.y - sun.r + (pass + .5) * sun.r * 2 / 3 + 2; sun.k = (pass + Math.max(0, f - .1) / .9) / 3; red.wz = Math.sin(i * .8) * 2; red.gesture = Prefs.shake ? { sx: 1 + .05 * Math.sin(i * .8), sy: 1 - .05 * Math.sin(i * .8) } : null;
    const a = -Math.PI / 2 + k * 6.28; tool.pinX = moon.x + Math.cos(a) * moon.r; tool.pinY = moon.y + Math.sin(a) * moon.r; tool.pinA = -2.4; moon.k = k; blue.wz = Math.sin(i * .6) * 2;
    if (i % 4 === 0) { B.particles.push({ wx: red.wx + R(-6, 6), wy: red.wy, wz: red.def.h, vx: 0, vy: 0, vz: R(1, 2), g: -.02, col: RE, t: 0, life: 16 }); B.particles.push({ wx: blue.wx + R(-6, 6), wy: blue.wy, wz: blue.def.h, vx: 0, vy: 0, vz: R(1, 2), g: -.02, col: BL, t: 0, life: 16 }); }
    if (i === 8 || i === 16) Audio.sfx('brush_sweep', { vol: .7 }); yield; }
  sun.k = moon.k = 1; red.gesture = null; users.forEach(u => u.wz = 0);
  for (let i = 1; i <= 6; i++) { tool.bro = 1 - i / 6; tool.pinX = moon.x + moon.r * .8 + i; tool.pinY = moon.y + moon.r * .3 + i; yield; } yield* wait(4);
  // el pincel arrastra la luna sobre el sol; la sombra tramada del eclipse barre el suelo hacia el objetivo
  Audio.sfx('hum_down'); Audio.sfx('brush_hiss', { vol: .5 }); const pc = partyC();
  const shade = fx(999, () => { const k = clamp(shade.t / 40, 0, 1), c = PJ([lerp(pc[0], t.wx, k), lerp(pc[1], t.wy, k), 0]); g.globalAlpha = .7; pixDisc(c[0], c[1], 70 * c[2], '#0b0912', null, null, 26 * c[2], 1); g.globalAlpha = 1; }, true);
  for (let i = 1; i <= 40; i++) { const k = i / 40, e = k < .5 ? 2 * k * k : 1 - Math.pow(-2 * k + 2, 2) / 2; moon.x = lerp(MOONX, SUNX, e); tool.pinX = moon.x + moon.r * .85; tool.pinY = moon.y + moon.r * .3; tool.pinA = -2.4 + Math.sin(i * .4) * .05; blue.wz = Math.sin(i * .5) * 1.5; if (i === 34) { Audio.sfx('glass'); B.hitstop = 6; } yield; }
  shade.dur = 0; st.corona = 1; Audio.sfx('mix', { colours: users.map(u => SEMI[u.id] ?? 0) }); B.flash = { col: VI, a: .5 }; B.slowmo = 14; B.shake = 3; blue.wz = 0;
  for (let i = 0; i < 18; i++) { st.corona = 1 + Math.sin(i * .5) * .3; tool.pin = Math.max(0, 1 - i / 6); yield; }
  pf.dur = 0;
  // totalidad: el disco violeta cae sobre el objetivo como un tampón, con estela, y se aplasta al chocar
  const drop = { k: 0, squash: 0 }, sh = shadowFx(t.wx, t.wy, 6, 999), goal = PJ(above(t, .5));
  const fall = fx(999, () => { const k = drop.k, x = lerp(SUNX, goal[0], k), y = lerp(SKYY, goal[1], k * k), r = lerp(16, 24, k);
    for (let j = 4; j >= 1; j--) { const kk = Math.max(0, k - j * .06); g.globalAlpha = .2 * (5 - j) / 4; pixDisc(lerp(SUNX, goal[0], kk), lerp(SKYY, goal[1], kk * kk), r, VI); } g.globalAlpha = 1;
    pixDisc(x, y, r * (1 + drop.squash * .35), VI, VR.out, VR.hi, r * (1 - drop.squash * .45)); });
  ...
  sh.dur = 0; Audio.sfx('impact_sub'); Audio.sfx('glass', { when: .1 }); B.hitstop = 12; B.shake = 9; B.slowmo = 16; B.flash = { col: PAPER, a: .6 }; impactFrame(t, VI, 1.5);
  ...
  const blast = fx(22, () => { const c = PJ(above(t, .6)), q = blast.t / 22; g.globalAlpha = q > .5 ? (1 - q) * 2 : 1; drawColorBlast(c[0], c[1], (8 + (1 - (1 - q) ** 3) * 40) * c[2], q, blast.t, [VR.dk, VI, VR.hi, PAPER]); g.globalAlpha = 1; });
  const ring = fx(24, () => { const c = project(t.wx, t.wy, 0); if (!c) return; const q = ring.t / 24; g.globalAlpha = 1 - q; pixRing(c[0], c[1], (8 + q * 90) * c[2], (3 + q * 34) * c[2], VI); pixRing(c[0], c[1], (6 + q * 84) * c[2], (2.5 + q * 32) * c[2], VR.dk); pixRing(c[0], c[1], (4 + q * 70) * c[2], (2 + q * 26) * c[2], PAPER); g.globalAlpha = 1; }, true);
  for (let i = 1; i <= 8; i++) { drop.squash = i <= 3 ? i / 3 : (8 - i) / 5; yield; } fall.dur = 0;
  mark({ kind: 'pool', p: [t.wx, t.wy + 2, 0], w: t.def.w * 1.1, col: VR.dk, grow: 3, life: 220, under: true }); mark({ kind: 'blob', p: [t.wx, t.wy, 0], w: t.def.w * .6, col: VI, grow: 3, life: 220, seed: 3, under: true });
  burst(t.wx, t.wy, 6, VI, 24, 2.6, 30, .1); burst(t.wx, t.wy, t.def.h, PAPER, 8, 1.5, 20, .05); drips(t.wx, t.wy, t.def.h * .9, VI, 10);
  yield* wait(24);
  // el cielo vuelve: los manchones se deshacen en partículas que regresan a cada uno
  dark.dur = Math.min(dark.dur, dark.t + 16); stars.dur = Math.min(stars.dur, stars.t + 12);
  for (let i = 0; i < 14; i++) { B.particles.push({ wx: red.wx + R(-10, 10), wy: red.wy, wz: red.def.h + 30 - i * 2, vx: 0, vy: 0, vz: -R(.6, 1.4), g: .02, col: RE, t: 0, life: 18 }); B.particles.push({ wx: blue.wx + R(-10, 10), wy: blue.wy, wz: blue.def.h + 30 - i * 2, vx: 0, vy: 0, vz: -R(.6, 1.4), g: .02, col: BL, t: 0, life: 18 }); yield; }
...
// Arcoíris, el prisma: los tres funden sus pigmentos en una gota que se vuelve blanca; la gota sube al cielo y Ámbar dibuja a lápiz un
// prisma a su alrededor, arista a arista; la página queda en blanco, el cristal se llena, recibe la luz desde arriba y la abre en seis
// haces hasta cada enemigo; se pintan de todos los colores y estallan; luego el arco cruza el cielo columna a columna y llueve color.
function* techArcoiris(users, targets, tech, col) {
  const ec = enemyC(), pc = partyC(), mid = [(ec[0] + pc[0]) / 2, (ec[1] + pc[1]) / 2];
  const RB = ['rojo', 'naranja', 'amarillo', 'verde', 'azul', 'violeta'].map(C), INK = '#241e32', PAPER = '#f4f0ea', yel = users.find(u => u.id === 'ambar') || users[0];
  // 1) fusión de los tres pigmentos entre el grupo: la gota de pintura se vuelve blanca
  camFocus(pc[0], pc[1], { dist: 96, turn: .35, h: 52, pitch: .55, hy: 84, ease: .1 }); users.forEach(u => u.pose = 'charge');
  const { of, orb } = yield* fuseAt(users, mid[0], mid[1], 22, PAPER); yield* wait(4);
  // 2) la gota blanca sube al cielo y Ámbar dibuja a lápiz el prisma a su alrededor, arista a arista
  camFocus(mid[0], mid[1], { dist: 120, turn: 0, h: 60, pitch: .42, f: 170, hy: 104, subjects: users, points: [[mid[0], mid[1], 72, 25]] }); // cámara baja mirando al cielo
  Audio.sfx('fwip', { semi: 4 }); const r0 = orb.r;
  for (let i = 1; i <= 14; i++) { const k = i / 14, e = 1 - (1 - k) ** 2; orb.wz = lerp(22, 72, e); orb.r = lerp(r0, 9, k); if (i % 2) B.particles.push({ wx: mid[0] + R(-4, 4), wy: mid[1] + R(-2, 2), wz: orb.wz - 6, vx: 0, vy: 0, vz: -R(.3, .8), g: .02, col: i % 4 ? PAPER : C(users[i % 3].color), t: 0, life: 14, size: 1 }); yield; }
  const prismP = () => PJ([mid[0], mid[1], 72]), tri = (c, s, a) => [[0, -1.1], [1, .7], [-1, .7]].map(([x, y]) => [c[0] + (x * Math.cos(a) - y * Math.sin(a)) * s, c[1] + (x * Math.sin(a) + y * Math.cos(a)) * s * .9]);
  const lap = propSprite('lapiz', C('amarillo')), pen = { k: 0, alpha: 0 };
  const draw = fx(999, () => { const c = prismP(), pts = tri(c, c[2] * 18, 0), q = pen.k * 3, full = Math.floor(q), f = q - full;
    for (let i = 0; i < 3; i++) { const a = pts[i], b = pts[(i + 1) % 3]; if (i < full) pstroke(a[0], a[1], b[0], b[1], 1, INK, 1, 0, false); else if (i === full && f > 0) pstroke(a[0], a[1], lerp(a[0], b[0], f), lerp(a[1], b[1], f), 1, INK, 1, 0, false); }
    if (pen.alpha > 0) { const i = Math.min(2, full), a = pts[i], b = pts[(i + 1) % 3], m = full > 2 ? 1 : f, tx = lerp(a[0], b[0], m), ty = lerp(a[1], b[1], m); drawProp(lap, tx, ty, -1.05, 1, PROP.lapiz.tip[0], PROP.lapiz.tip[1], .7 * c[2], pen.alpha); } });
  { const ys = PJ(above(yel, 1.2)); sparkle(...above(yel, 1.7), '#fff3c0'); Audio.sfx('scratch_long'); yel.pose = 'attack';
    for (let i = 1; i <= 8; i++) { pen.alpha = i / 8; yel.wz = i; yield; }
    for (let i = 1; i <= 18; i++) { pen.k = i / 18; yel.wz = 8 + Math.sin(i * .9) * 2; yel.gesture = Prefs.shake ? { sx: 1 + .04 * Math.sin(i * .9), sy: 1 - .04 * Math.sin(i * .9) } : null; if (i === 6 || i === 12) Audio.sfx('scratch', { vol: .6 }); yield; }
    yel.gesture = null; for (let i = 1; i <= 5; i++) { pen.alpha = 1 - i / 5; yel.wz = 8 - i * 1.6; yield; } yel.wz = 0; }
  Audio.sfx('mix_bell', { vol: .6 }); B.slowmo = 8; for (let i = 1; i <= 6; i++) { orb.r = 9 * (1 - i / 6); yield; } of.dur = 0; draw.dur = 0;
  ...
  // 3) papel: la pantalla se vuelve hoja en blanco y el dibujo se llena de cristal tramado
  const paper = overlay(999, '#f1e9d6', .82, 12, 18); B.flash = { col: '#ffffff', a: .9 }; Audio.sfx('glass'); Audio.sfx('rainbow');
  const pr = { spin: 0, k: 0, beams: 0 };
  const prism = fx(999, () => { const c = prismP(), s = c[2] * 18, pts = tri(c, s, pr.spin);
  ...
  bf.dur = 0; users.forEach(u => u.pose = 'happy'); for (let i = 1; i <= 14; i++) { users.forEach((u, j) => { u.wz = Math.max(0, Math.sin((i + j * 2) / 14 * Math.PI)) * 8; }); yield; }
  users.forEach(u => { u.wx = u.hx; u.wy = u.hy; u.wz = 0; }); yield* wait(20); users.forEach(u => u.pose = 'idle'); camReset();
}
// ---- Nivel 2 del Eclipse: Eclipse total. Cae la noche sobre toda la página; Carmín y Añil alzan juntos un sol rojo y una luna azul
// que se funden en un disco negro con corona violeta justo encima del enemigo. El agujero le absorbe la tinta (hilos que suben
// en espiral y lo dejan pálido y encogido) y, cuando está lleno, cae entero sobre él: sello violeta, anillos y Lento.
function* techEclipseTotal(users, t, tech, col) {
  const VI = C('violeta'), RE = C('rojo'), BL = C('azul'), VR = ramp(VI), RR = ramp(RE), BR = ramp(BL), PAPER = '#f4f0ea', INK = '#0b0912';
  const red = users.find(u => u.id === 'carmin') || users[0], blue = users.find(u => u.id === 'anil') || users[1] || users[0];
  camFocus(t.wx, t.wy, { dist: 96, turn: -.2, h: 60, pitch: .5, f: 165, headroom: 70, subjects: [t] }); users.forEach(u => u.pose = 'charge');
  const night = overlay(999, '#07050c', .82, 30, 16), st = { sun: 0, moon: 0, merge: 0, corona: 0, hole: 0, full: 0, fall: 0, squash: 0 };
  const stars = fx(999, () => { const k = clamp(stars.t / 30, 0, 1), rnd = seeded(91); for (let i = 0; i < 40; i++) { const x = rnd() * W, y = rnd() * 110, tw = (Math.sin(stars.t * .2 + i * 1.7) + 1) * .5; if (rnd() < k) { g.globalAlpha = k * (.3 + tw * .7); g.fillStyle = i % 7 ? PAPER : VR.hi; g.fillRect(x | 0, y | 0, 1, 1); if (i % 9 === 0) { g.fillRect((x | 0) - 1, y | 0, 3, 1); g.fillRect(x | 0, (y | 0) - 1, 1, 3); } } } g.globalAlpha = 1; });
  // el disco vive sobre el enemigo, en el cielo del plano
  const at = () => { const c = PJ([t.wx, t.wy, t.def.h * (t.boss ? 1.35 : 1.6) + 40]); return [c[0], Math.max(26, c[1]), c[2]]; };
  const sky = fx(999, () => {
    const [cx, cy, s] = at(), R0 = 17 * Math.max(1, s * .9), sep = (1 - st.merge) * 34;
    if (st.sun > 0 && st.merge < 1) { pixDisc(cx - sep, cy, R0 * st.sun, RE, RR.out, RR.hi); for (let i = 0; i < 12; i++) { const a = i * .52 + sky.t * .04, L = R0 * st.sun * (1.35 + .2 * Math.sin(sky.t * .3 + i)); pstroke(cx - sep + Math.cos(a) * (R0 + 2), cy + Math.sin(a) * (R0 + 2), cx - sep + Math.cos(a) * L, cy + Math.sin(a) * L, 1, RR.hi, 1, 0, false); } }
    if (st.moon > 0 && st.merge < 1) { pixDisc(cx + sep, cy, R0 * st.moon, BL, BR.out, BR.hi); pixDisc(cx + sep + R0 * .3, cy + R0 * .2, R0 * .2 * st.moon, BR.sh); }
    if (st.corona > 0) { for (let i = 0; i < 24; i++) { const a = i * .262 + sky.t * .03, L = R0 * (1.3 + st.corona * (1 + .35 * Math.sin(sky.t * .35 + i * 2.3))); pstroke(cx + Math.cos(a) * (R0 + 1), cy + Math.sin(a) * (R0 + 1), cx + Math.cos(a) * L, cy + Math.sin(a) * L, i % 3 ? 1 : 2, i % 2 ? VI : PAPER, 1, 0, false); } g.globalAlpha = .25 * st.corona; pixDisc(cx, cy, R0 * 2.6, VI); g.globalAlpha = 1; }
    if (st.hole > 0) { pixDisc(cx, cy, R0 * (1 + st.full * .25), INK, VR.out); pixRing(cx, cy, R0 * .7 * (1 + st.full * .3), R0 * .7 * (1 + st.full * .3), VR.dk); g.fillStyle = VR.hi; g.fillRect(Math.round(cx - R0 * .5), Math.round(cy - R0 * .6), 3, 1); }
  });
  // 1) los dos alzan su astro: Carmín el sol, Añil la luna
  Audio.sfx('hum_down'); Audio.sfx('charge', { semi: 0 }); Audio.sfx('charge', { semi: 7, when: .12 }); sparkle(...above(red, 1.5), RR.hi); sparkle(...above(blue, 1.5), BR.hi);
  for (let i = 1; i <= 22; i++) { st.sun = Math.min(1, i / 14); st.moon = clamp((i - 6) / 14, 0, 1); red.wz = Math.sin(i * .5) * 2; blue.wz = Math.cos(i * .5) * 2; if (i % 4 === 0) { B.particles.push({ wx: red.wx, wy: red.wy, wz: red.def.h, tx: t.wx, ty: t.wy, tz: 60, col: RE, t: 0, life: 16, dur: 16, arc: 10, stream: true, size: 2 }); B.particles.push({ wx: blue.wx, wy: blue.wy, wz: blue.def.h, tx: t.wx, ty: t.wy, tz: 60, col: BL, t: 0, life: 16, dur: 16, arc: 10, stream: true, size: 2 }); } yield; }
  users.forEach(u => u.wz = 0);
  // 2) se juntan y se tapan: totalidad, corona y el disco se vuelve un agujero negro
  Audio.sfx('brush_hiss', { vol: .5 });
  for (let i = 1; i <= 20; i++) { st.merge = (i / 20) ** 2; yield; }
  Audio.sfx('glass'); Audio.sfx('mix', { colours: users.map(u => SEMI[u.id] ?? 0) }); B.flash = { col: VI, a: .5 }; B.hitstop = 6; B.shake = 3; st.hole = 1;
  for (let i = 1; i <= 12; i++) { st.corona = i / 12; yield; }
  // 3) absorbe: la tinta del enemigo sube en hilos espirales hasta el agujero; el enemigo palidece y se encoge
  camFocus(t.wx, t.wy, { dist: 84, turn: -.25, h: 54, pitch: .5, f: 165, headroom: 64, subjects: [t] }); Audio.sfx('ink_tide'); Audio.sfx('saturate', { when: .3, vol: .5 });
  const strands = fx(999, () => { const [cx, cy] = at(), b = PJ(above(t, .5)); for (let j = 0; j < 5; j++) { const ph = strands.t * .15 + j * 1.26; let px = b[0], py = b[1]; for (let k = 1; k <= 12; k++) { const q = k / 12, x = lerp(b[0], cx, q) + Math.sin(ph + q * 8) * 10 * (1 - q), y = lerp(b[1], cy, q * q); pstroke(px, py, x, y, Math.max(1, 3 - q * 2), j % 2 ? '#1e1a2c' : VR.dk, 1, 0, false); px = x; py = y; } } }, true);
  for (let i = 1; i <= 34; i++) { st.full = i / 34; t.sqz = i / 34 * .8; t.sketch = i > 20 ? 1 : 0; if (i % 2) B.particles.push({ wx: t.wx + R(-8, 8), wy: t.wy, wz: t.def.h * R(.3, 1), tx: t.wx, ty: t.wy, tz: 64, col: i % 4 ? '#1e1a2c' : VI, t: 0, life: 14, dur: 14, arc: 6, stream: true, size: 2 }); if (i % 6 === 0) { Audio.sfx('slow_drip', { vol: .5 }); B.shake = 1; } yield; }
  strands.dur = 0; t.sketch = 0;
  // 4) el agujero lleno cae entero sobre el enemigo como un tampón y lo sella en violeta
  Audio.sfx('drop_fall'); const sh = shadowFx(t.wx, t.wy, 6, 999); sky.dur = 0; stars.dur = Math.min(stars.dur, stars.t + 20);
  const fall = fx(999, () => { const [cx, cy, s] = at(), g2 = PJ(above(t, .45)), y = lerp(cy, g2[1], st.fall ** 2), r = 17 * Math.max(1, s * .9) * 1.25 * (1 + st.fall * .3);
    for (let j = 4; j >= 1; j--) { const kk = Math.max(0, st.fall - j * .07); g.globalAlpha = .18 * (5 - j) / 4; pixDisc(cx, lerp(cy, g2[1], kk * kk), r, VI); } g.globalAlpha = 1;
    pixDisc(cx, y, r * (1 + st.squash * .4), VI, VR.out, VR.hi, r * (1 - st.squash * .5)); pixDisc(cx, y, r * .55 * (1 - st.squash * .3), INK); });
  for (let i = 1; i <= 10; i++) { st.fall = i / 10; sh.draw = () => { const c = project(t.wx, t.wy, 0); if (c) shadow(c[0], c[1], Math.round((8 + st.fall * 34) * c[2])); }; yield; }
  sh.dur = 0; t.sqz = 0; Audio.sfx('impact_sub'); Audio.sfx('glass', { when: .08 }); B.hitstop = 14; B.shake = 10; B.slowmo = 18; B.flash = { col: PAPER, a: .7 }; impactFrame(t, VI, 1.6);
  damage(t, baseDmg(users.reduce((s, u) => s + u.atk * statusMult(u, 'tiznado'), 0) / users.length, t.dfn, tech.power), col, tech.name); if (tech.status) applyStatus(t, tech.status, 3); goop(t, VI, 140); lensSplatter(VI, 8, 11); lensSplatter('#1e1a2c', 4, 12);
  const blast = fx(26, () => { const c = PJ(above(t, .6)), q = blast.t / 26; g.globalAlpha = q > .5 ? (1 - q) * 2 : 1; drawColorBlast(c[0], c[1], (10 + (1 - (1 - q) ** 3) * 50) * c[2], q, blast.t, [INK, VR.dk, VI, VR.hi, PAPER]); g.globalAlpha = 1; });
  for (let k = 0; k < 3; k++) { const ring = fx(30, () => { const c = project(t.wx, t.wy, 0); if (!c) return; const q = clamp((ring.t - k * 5) / 25, 0, 1); if (q <= 0) return; g.globalAlpha = 1 - q; pixRing(c[0], c[1], (8 + q * 110) * c[2], (3 + q * 42) * c[2], [VI, VR.hi, PAPER][k]); g.globalAlpha = 1; }, true); }
  for (let i = 1; i <= 9; i++) { st.squash = i <= 3 ? i / 3 : (9 - i) / 6; yield; } fall.dur = 0;
  mark({ kind: 'pool', p: [t.wx, t.wy + 2, 0], w: t.def.w * 1.3, col: VR.dk, grow: 3, life: 260, under: true }); mark({ kind: 'blob', p: [t.wx, t.wy, 0], w: t.def.w * .7, col: VI, grow: 3, life: 260, seed: 4, under: true });
  for (let j = 0; j < 8; j++) { const a = j / 8 * 6.28; mark({ kind: 'path', pts: [[t.wx, t.wy, 0], [t.wx + Math.cos(a) * t.def.w, t.wy + Math.sin(a) * t.def.w * .45, 0]], w: 2, col: VI, grow: 4, life: 240, under: true }); }
  burst(t.wx, t.wy, 6, VI, 26, 2.8, 30, .1); burst(t.wx, t.wy, t.def.h, PAPER, 10, 1.6, 22, .05); drips(t.wx, t.wy, t.def.h * .9, VI, 12);
  yield* wait(20);
  // 5) amanece: la noche se retira
  night.dur = Math.min(night.dur, night.t + 16); stars.dur = Math.min(stars.dur, stars.t + 10);
  users.forEach(u => u.pose = 'happy'); yield* wait(18); users.forEach(u => u.pose = 'idle'); camReset();
}
// ---- Nivel 2 del Arcoíris: Espectro. Los tres pintan juntos un puente de arcoíris que nace a sus pies y aterriza sobre los enemigos;
// por él bajan seis ríos de color, uno tras otro, que rompen sobre la fila en olas de su color. Al final el puente se rompe en
// gotas de seis colores que llueven sobre todo el campo.
function* techEspectro(users, targets, tech, col) {
  const ec = enemyC(), pc = partyC(), RB = ['rojo', 'naranja', 'amarillo', 'verde', 'azul', 'violeta'].map(C), PAPER = '#f4f0ea';
  const dmg = t => damage(t, baseDmg(users.reduce((s, u) => s + u.atk * statusMult(u, 'tiznado'), 0) / users.length, t.dfn, tech.power), col, tech.name);
  camFocus(pc[0], pc[1], { dist: 96, turn: .35, h: 52, pitch: .55, hy: 84, ease: .1 }); users.forEach(u => u.pose = 'charge');
  const { of } = yield* fuseAt(users, pc[0], pc[1], 22, PAPER); yield* wait(4); of.dur = 0;
  // el puente: un arco de mundo de la fila del grupo a la de los enemigos; seis bandas paralelas
  const A = [pc[0], pc[1]], Z = [ec[0], ec[1]], span = Math.hypot(Z[0] - A[0], Z[1] - A[1]), H0 = Math.max(60, span * .55);
  const at = (k, band) => { const off = (band - 2.5) * 2.2; return [lerp(A[0], Z[0], k) + B.axis.rx * off, lerp(A[1], Z[1], k) + B.axis.ry * off, Math.sin(k * Math.PI) * H0 + 4]; };
  camFocus(lerp(A[0], Z[0], .5), lerp(A[1], Z[1], .5), { dist: 150, turn: .95, h: 64, pitch: .5, f: 170, hy: 104, ease: .07, subjects: [...users, ...targets], points: [[lerp(A[0], Z[0], .5), lerp(A[1], Z[1], .5), H0, 12]] }); // de lado: el puente cruza el plano entero
  const br = { k: 0, alpha: 1, rivers: [], crack: 0 };
  const bf = fx(999, () => {
    const n = 40; g.globalAlpha = br.alpha;
    for (let b = 0; b < 6; b++) { let prev = null; for (let i = 0; i <= n * br.k; i++) { const k = i / n, c = PJ(at(k, b)); if (prev) { const gap = br.crack > 0 && ((i * 7 + b * 3) % 11) / 11 < br.crack; if (!gap) pstroke(prev[0], prev[1], c[0], c[1], Math.max(2, 3.2 * c[2]), RB[b], 1, 0, false); } prev = c; } }
    // brillo del borde de arriba y contorno de tinta del de abajo
    let p0 = null, p1 = null; for (let i = 0; i <= n * br.k; i++) { const k = i / n, a = PJ(at(k, -.6)), z = PJ(at(k, 5.6)); if (p0 && br.crack < .3) { pstroke(p0[0], p0[1], a[0], a[1], 1, PAPER, 1, 0, false); pstroke(p1[0], p1[1], z[0], z[1], 1, '#2a2438', 1, 0, false); } p0 = a; p1 = z; }
    // los ríos: una lengua brillante de cada color que baja por su banda
    for (const r of br.rivers) { if (r.k <= 0 || r.k >= 1.15) continue; for (let j = 0; j < 6; j++) { const k = Math.min(1, r.k - j * .025); if (k <= 0) continue; const c = PJ(at(k, r.b)); pixDisc(c[0], c[1], Math.max(1.5, (4 - j * .5) * c[2]), j ? RB[r.b] : PAPER, j ? null : ramp(RB[r.b]).out); } }
    g.globalAlpha = 1;
  });
  // 1) los tres lo pintan: cada uno sube su color y el arco crece desde sus pies
  Audio.sfx('rainbow'); users.forEach(u => u.pose = 'attack');
  for (let i = 1; i <= 30; i++) { br.k = 1 - (1 - i / 30) ** 2; users.forEach((u, j) => { u.wz = Math.abs(Math.sin(i * .5 + j)) * 4; }); if (i % 5 === 0) Audio.sfx('cursor', { semi: [0, 2, 4, 5, 7, 9][(i / 5 - 1) % 6], vol: .5 }); if (i % 2) { const c = at(br.k, RI(0, 5)); B.particles.push({ wx: c[0], wy: c[1], wz: c[2], vx: R(-.3, .3), vy: 0, vz: R(-.2, .4), g: .02, col: '#ffffff', t: 0, life: 12, size: 1 }); } yield; }
  users.forEach(u => u.wz = 0);
  // 2) seis ríos de color bajan uno tras otro y rompen en olas sobre la fila
  camFocus(ec[0], ec[1], { dist: 104, turn: .5, h: 56, pitch: .54, ease: .1, subjects: targets, headroom: 30, points: [[lerp(A[0], Z[0], .75), lerp(A[1], Z[1], .75), H0 * .7, 10]] });
  const waves = [], wf = fx(999, () => { for (const w of waves) { const q = (wf.t - w.at) / 24; if (q < 0 || q > 1) continue; for (const t of targets) { const c = PJ([t.wx, t.wy, 0]), r = (10 + q * 40) * c[2]; g.globalAlpha = (1 - q) * .9; g.fillStyle = w.col; g.beginPath(); g.ellipse(c[0], c[1] - (1 - q) * 6, r, r * .38, 0, Math.PI, 0); g.fill(); g.fillStyle = ramp(w.col).hi; g.fillRect(Math.round(c[0] - r * .6), Math.round(c[1] - r * .38 - (1 - q) * 6), Math.round(r * 1.2), 1); } } g.globalAlpha = 1; });
  br.rivers = RB.map((_, b) => ({ b, k: -b * .18 }));
  const landed = new Set(); let hitDone = false;
  for (let i = 0; i < 64; i++) {
    for (const r of br.rivers) { const was = r.k; r.k += 1 / 22; if (was < 1 && r.k >= 1) { waves.push({ col: RB[r.b], at: wf.t }); Audio.sfx('splash_clean', { semi: r.b * 2, vol: .7 }); B.shake = Math.max(B.shake, 3);
      for (const t of targets) { t.goop = null; goop(t, RB[r.b], 40); burst(t.wx, t.wy, t.def.h * .5, RB[r.b], 8, 1.8, 22, .08); mark({ kind: 'pool', p: [t.wx + R(-8, 8), t.wy + R(-4, 4), 0], w: R(6, 11), col: RB[r.b], grow: 6, life: 220, under: true }); }
      if (r.b === 5 && !hitDone) { hitDone = true; B.hitstop = 8; B.slowmo = 12; B.flash = { col: '#ffffff', a: .6 }; impactFrame(targets, PAPER, 1.4); for (const t of targets) { dmg(t); const c = PJ([t.wx, t.wy, t.def.h * .6]); fx(24, function () { const q = this.t / 24; g.globalAlpha = q > .5 ? (1 - q) * 2 : 1; drawColorBlast(c[0], c[1], (8 + (1 - (1 - q) ** 3) * 38) * c[2], q, this.t, RB); g.globalAlpha = 1; }); } lensSplatter(RB[0], 3, 5); lensSplatter(RB[2], 3, 6); lensSplatter(RB[4], 3, 7); } } }
    yield;
  }
  // 3) el puente se rompe en gotas de seis colores que llueven sobre todo el campo
  Audio.sfx('glass'); Audio.sfx('heal_bells', { when: .3 }); B.rainbow = 30;
  for (let i = 1; i <= 30; i++) { br.crack = i / 22; br.alpha = Math.max(0, 1 - Math.max(0, i - 14) / 16); if (i % 2) for (let j = 0; j < 3; j++) { const c = at(Math.random(), RI(0, 5)); B.particles.push({ wx: c[0], wy: c[1], wz: c[2], vx: R(-.3, .3), vy: 0, vz: -R(.5, 1.2), g: .06, col: RB[RI(0, 5)], t: 0, life: 40, size: 2 }); } yield; }
  bf.dur = 0; wf.dur = 0;
  users.forEach(u => u.pose = 'happy'); for (let i = 1; i <= 14; i++) { users.forEach((u, j) => { u.wz = Math.max(0, Math.sin((i + j * 2) / 14 * Math.PI)) * 8; }); yield; }
  users.forEach(u => { u.wx = u.hx; u.wy = u.hy; u.wz = 0; }); yield* wait(14); users.forEach(u => u.pose = 'idle'); camReset();
}
// =====================================================================
...
function* actAttack(u, target) {
  const t = target;
  if (u.kind !== 'party' && u.data.weapon) { const w = u.data.weapon; if (w === 'brocha') yield* atkBrocha(u, t); else if (w === 'lapiz') yield* atkLapiz(u, t); else yield* atkPincel(u, t); u.pose = 'idle'; return; } // los Contrarios también llevan su herramienta
...
function* actTech(users, tech, targets, col) {
  const id = tech.id || Object.keys(DATA.techs).find(k => DATA.techs[k] === tech), lv2 = tech.level === 2;
  ...
  else if (id === 'llamarada') yield* (lv2 ? techLlamaradaFenix : techLlamarada)(users, targets, tech, col);
  else if (id === 'brote') yield* (lv2 ? techSelva : techBrote)(users, targets, tech, col);
  else if (id === 'eclipse') yield* (lv2 ? techEclipseTotal : techEclipse)(users, targets[0], tech, col);
  else if (id === 'arcoiris') yield* (lv2 ? techEspectro : techArcoiris)(users, targets, tech, col);
...
function* atkPluma(u, t) {
  ...
  camFocus(t.wx,t.wy,{dist:76,turn:-.2}); u.pose='attack'; yield* whop(u, ...infront(u, t, t.def.w * .7 + 20), 9, 12); Audio.sfx('scratch');
  ...
  yield* wait(14); yield* returnHome(u, 10); yield* wait(4); camReset();
}
