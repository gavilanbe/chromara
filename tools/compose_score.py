#!/usr/bin/env python3
"""Chromara v3: three original scene cues and a pitched SNES effects palette.

python tools/compose_score.py --soundfont '/path/Chrono Trigger.sf2'
Requires FluidSynth (system library) and tools/audio_requirements.txt.
The existing six v2 MIDI/WAV masters remain the source for the main themes.
"""
import argparse
import base64
import ctypes as C
...
def main():
    ...
    synth=Synth(args.soundfont); meta=json.loads((MUSIC/'score/v2-meta.json').read_text());fallback={}
    for score in [atelier(),prelude(),restored()]:
        score.midi();meta[score.name]=render_score(score,synth,args.render_dir/(score.name+'.wav'))
        fallback[score.name]={'bpm':score.bpm,'stepsPerBeat':4,'length':(score.intro+score.bars)*score.meter*4,
                             'loop':True,'tracks':[]}
        # Emergency voices use the new score, with a single intro per iteration.
        for voice,notes in score.voices.items():
            program,level,pan,send=PALETTE[voice]
            fallback[score.name]['tracks'].append({'wave':'sine' if voice in ['harp','marimba'] else 'tri',
                'vol':round(level*.10,3),'env':[.018,.10,.55,.12],'echo':send,
                'notes':[[t*4,p,d*4,v/127] for t,d,p,v in notes if t<(score.intro+score.bars)*score.meter]})
        print(score.name,meta[score.name]['duration'],'s, voices',meta[score.name]['maxWrittenPolyphony'],flush=True)
    (MUSIC/'meta.json').write_text(json.dumps(meta,ensure_ascii=False,indent=2)+'\n')
    (ROOT/'music_fallback.js').write_text('// Generated from the original v3 scene scores.\nconst MUSIC_FALLBACK = '+json.dumps(fallback,separators=(',',':'))+';\n')
