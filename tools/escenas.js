...
const ESCENAS = {
  ...
  // ?escena=jefa_plano&fase=1..3: plano fijo de La Tinta, para revisar su presencia (tentáculos y goterones).
  jefa_plano() { escenaBattle('B', false); const b = B.enemies[0], f = +(new URLSearchParams(location.search).get('fase') || 1); b.bossPhase = f; B.enemies.forEach(u => u.atb = 0); B.party.forEach(u => u.atb = 0); Prefs.camera = 'fija'; camSet(fitCameraSubjects([b], { home: true, dist: 80, zoom: 1.2, headroom: 20, box: [30, 30, 290, 140] })); B.viewKey = 'plano'; updateMenuCamera = () => {}; },
